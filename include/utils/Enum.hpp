...
namespace utils {
    ...
    struct static_string {
        static constexpr std::size_t static_string_npos = INT_MAX;// std::numeric_limits<int>::max();
...
#define DECLARE_ENUM(T, ...) \
    ...
    private:                      \
        ...
        static constexpr auto enum_buffer = utils::make_static_string(#__VA_ARGS__);                                    \
        static constexpr std::size_t enum_size = (enum_buffer.count(',', 0) + 1);                     \
        static inline std::array<std::string, enum_size> m_arr         \
            = utils::unpack_enum_string<T, enum_size>(enum_buffer); \
    };

#define ENUM2STR_BASE(Namespace, value) Namespace::enum_classes_holder::get_string<std::decay_t<decltype(value)>>(static_cast<std::underlying_type_t<std::decay_t<decltype(value)>>>(value))
#define ENUM2STR(value) utils::enum_classes_holder::get_string<std::decay_t<decltype(value)>>(static_cast<std::underlying_type_t<std::decay_t<decltype(value)>>>(value))