...
namespace utils {
    ...
    namespace util::helper {
        ...
        template<typename U>
        struct formatter<Result<U>> {
            static std::string to_string(const Result<U>& result) {
    ...
    template<typename ... Args>
    void log_ln(std::string&& formatMessage, Args&& ... args) {
#ifdef COMMON_LOG_DEBUG
        std::cout << util::format_string(std::move(formatMessage), std::forward<Args>(args)...) << std::endl;
#endif // COMMON_LOG_DEBUG
    }


    template<typename ... Args>
    void msg(std::string&& formatMessage, Args&& ... args) {
        std::cout << util::format_string(std::move(formatMessage), std::forward<Args>(args)...) << std::endl;
    }
